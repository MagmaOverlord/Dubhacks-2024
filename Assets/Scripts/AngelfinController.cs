...
public class AngelfinController : MonoBehaviour
{
    ...
    {
        if (!isDead) {
            horizontal = Input.GetAxis("Horizontal2") * speed;
            if (IsGrounded && Input.GetButtonDown("Jump2"))
                jump = true;
            jumpHeld = !IsGrounded && Input.GetButton("Jump2");
            ...
            if (!IsGrounded)
            {
                ...
                {
                    GetComponent<Animator>().Play("AngelfinFall");
                }
                ...
                GetComponent<Animator>().Play("AngelfinIdle");
            else
                GetComponent<Animator>().Play("AngelfinMove");
            }
    }
    ...
    private void PlayerDeath() {
        ...
        GetComponent<Animator>().Play("AngelfinDeath");
    ...
    public void PlayStep() {
        int soundIndex = Random.Range(0, footsteps.Count);
        AudioSource.PlayClipAtPoint(footsteps[soundIndex],  transform.position);
    }
